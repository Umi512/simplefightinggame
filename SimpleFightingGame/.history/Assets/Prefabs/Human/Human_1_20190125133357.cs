...
public class Human_1 : MonoBehaviour 
{
	...
	//if player hit floor
	void OnTriggerEnter2D(Collider2D col)////bullet打つとキャラが初期場所に移動しない
	{
		...
		if(col.tag=="hand"||col.tag=="DeadLine"||(col.tag=="Bullet"&&tag!=BulletController1.getPlayerTag))
		{
			/* 
			//Playerの初期位置のどちらかに復活
	...
	void Update () 
	{
		if((int)UIFightingController.startCount==0)
		{
			...
			if (Input.GetKeyDown (KeyCode.S)) {
				...
				BulletController1.getPlayerTag=tag;
			}
		}
	}
}
